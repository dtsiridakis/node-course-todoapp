  ...
  it('should create a new todo', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => { // Now we must check what we have store on mongodb.Thats why we require Todo up above
        ...
        Todo.find({text}).then((todos) => { // We find all the todos inside the collection
...
describe('GET /todos', () => {
  it('Should show todos properly', (done) => {
    request(app)
      ...
      .expect(200)
      .expect((res) => {
        expect(res.body.dbfiles.length).toBe(1)
      })
      .end(done);
  });
});
...
describe('PATCH /todo/:id', () => {
  ...
  it('Should NOT update todo from othe user!', (done) => {
    var id = todos[0]._id;
    var update = {text: 'Updated', completed: true}
...
describe('POST /users', () => {
  it('Should create a user', (done) => {
    ...
    request(app)
      ...
      .expect(200)
      .expect((res) => {
        expect(res.headers['x-auth']).toBeTruthy(); // we select with headers['x-auth']
        expect(res.body._id).toBeTruthy();    // because this headers.x-auth is not valid!
...
describe('DELETE /users/me/token', () => {
  it('Should delete the token from user', (done) => {
    request(app)
